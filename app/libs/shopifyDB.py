...
class ShopifyID:
    ...
    def toCsv (self) -> str:
        ...
        # Build the CSV Line
        returnValue += str(self.md5)      + DELIMITER
        returnValue += str(self.shIdProd)    + DELIMITER
        returnValue += str(self.shIdVariant) + DELIMITER
        returnValue += self.sthId            + DELIMITER

        returnValue += self.status           + DELIMITER

        if (self.entity == SH_ENTITY.NONE ): returnValue        +=  'UNKNOWN'   + DELIMITER
        if (self.entity == SH_ENTITY.PRODUCT): returnValue      +=  'PRODUCT'   + DELIMITER
        if (self.entity == SH_ENTITY.CUSTOMER ): returnValue    +=  'CUSTOMER'  + DELIMITER
        if (self.entity == SH_ENTITY.METAOBJ): returnValue      +=  'METAOBJ'   + DELIMITER
